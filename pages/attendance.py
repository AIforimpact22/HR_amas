...
def fetch_day(day: datetime.date) -> pd.DataFrame:
    ...
    df["late"] = df.apply(
        lambda r: False
        if r.expected_in is None
        else r.clock_in.time()
        > (
...
def fetch_range(eid: int, s: datetime.date, e: datetime.date) -> pd.DataFrame:
    ...
    df["late"] = df.apply(
        lambda r: False
        if r.expected_in is None
        else pd.to_datetime(r.clock_in).time()
        > (
            datetime.datetime.combine(datetime.date.today(), r.expected_in)
            + datetime.timedelta(minutes=5)
        ).time(),
...
with tab_log:
    ...
    def sty(row):
        ...
        if row["Req IN"] != "—":
            ...
            cut = (
                datetime.datetime.combine(today, exp) + datetime.timedelta(minutes=5)
            ).time()
            style[1] = (
...
with tab_sched:
    ...
    view = pd.DataFrame(
        {
            ...
            "Off‑day": sch.off_day.map(
                lambda i: dow[i] if 0 <= i < 7 else "—"
            ),
    ...
    head_cols = st.columns(len(hdrs))
    for c, h in zip(head_cols, hdrs):
        c.markdown(f"**{h}**")

    # table rows
    for _, row in view.iterrows():
        cols = st.columns(len(hdrs))
        for i, field in enumerate(view.columns[:-1]):  # exclude att_id
            cols[i].markdown(str(row[field]))
        if cols[-1].button("✏️ Edit", key=f"edit_{row.att_id}"):
            st.session_state["edit_row"] = int(row.att_id)

    # inline edit form (unique keys per record)
    if (
        "edit_row" in st.session_state
        and st.session_state["edit_row"] in set(sch.att_id)
    ):
        ...
        with st.form(f"form_{rid}"):
            ...
            off = st.selectbox(
                "Off‑day",
                dow,
                index=rec.off_day if 0 <= rec.off_day < 7 else 0,
                key=f"off_{rid}",
            )

            cin_default = (
                rec.clock_in if pd.notna(rec.clock_in) else datetime.time(8, 0)
            )
            cout_default = (
                rec.clock_out if pd.notna(rec.clock_out) else datetime.time(17, 0)
            )
            cin = st.time_input("Clock‑in", cin_default, key=f"cin_{rid}")
            cout = st.time_input("Clock‑out", cout_default, key=f"cout_{rid}")

            efff_default = (
                rec.effective_from.date()
                if hasattr(rec.effective_from, "date")
                else datetime.date.today()
            )
            efff = st.date_input(
                "Effective from", efff_default, key=f"efff_{rid}"
            )

            efft_default = (
                rec.effective_to.date()
                if pd.notna(rec.effective_to)
            ...
            rsn = st.text_area(
                "Reason", rec.reason or "", key=f"rsn_{rid}"
            )
        ...
        # purge dangling state (row no longer exists)
