...
with tab_sched:
    ...
    # header row
    header_cols = st.columns(len(view.columns) - 1 + 1)
    ...
    for _, r in view.iterrows():
        row_cols = st.columns(len(view.columns) - 1 + 1)
        for i, field in enumerate(view.columns[:-1]):
            row_cols[i].markdown(str(r[field]))
        if row_cols[-1].button("✏️ Edit", key=f"edit_{r.att_id}"):
    ...
    if "edit_row" in st.session_state:
        ...
        with st.form("edit_form"):
            wd    = st.number_input("Work days / week", 1, 7,
                                    int(rec.wd_per_wk), key="wd")
            off   = st.selectbox("Off‑day", dow,
                                 index=rec.off_day if 0<=rec.off_day<7 else 0,
                                 key="off")
            cin0  = rec.clock_in if pd.notna(rec.clock_in) else datetime.time(8,0)
            cout0 = rec.clock_out if pd.notna(rec.clock_out) else datetime.time(17,0)
            cin   = st.time_input("Clock‑in",  cin0,  key="cin")
            cout  = st.time_input("Clock‑out", cout0, key="cout")
            efff0 = rec.effective_from if pd.notna(rec.effective_from) else datetime.date.today()
            efff  = st.date_input("Effective from", efff0, key="efff")
            efft0 = (rec.effective_to
                     if pd.notna(rec.effective_to)
                     else datetime.date(2100,1,1))
            efft  = st.date_input("Effective to (blank = open)", efft0, key="efft")
            efft  = None if efft == datetime.date(2100,1,1) else efft
            rsn   = st.text_area("Reason", rec.reason or "", key="rsn")
