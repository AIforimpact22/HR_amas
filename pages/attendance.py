...
with tab_sched:
    ...
    if "edit_row" in st.session_state:
        ...
        with st.form(form_key):
            ...
            efff_default = rec.effective_from.date() if hasattr(rec.effective_from, "date") else datetime.date.today()
            efff = st.date_input("Effective from", efff_default, key=f"efff_{rid}")

            efft_default = (
                rec.effective_to.date() if pd.notna(rec.effective_to)
